<x-layout :posts="$posts">
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="text-center mb-12 border-b-2 border-border pb-8">
            <h1 class="text-6xl md:text-8xl font-black text-text tracking-tighter leading-none mb-2 newspaper-title">All
                News</h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            ...
            <div
                class="group bg-card p-6 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 border border-border flex flex-col">
                ...
                <a href="/blog?category={{ $post->category->slug }}"
                    class="text-xs font-semibold text-primary hover:underline uppercase tracking-wider">{{ $post->category->name }}</a>
                @endif
                <h3 class="text-2xl font-bold text-text mt-2 mb-3">
                    <a href="/post/{{ $post->slug }}"
                        class="hover:text-primary transition-colors duration-300">{{ $post->title }}</a>
                </h3>
                ...
                <div class="flex justify-between items-center mt-auto pt-4 border-t border-border/50">
                    ...
                    <a href="/post/{{ $post->slug }}"
                        class="inline-flex items-center text-sm font-semibold text-primary transition-all duration-300 ease-in-out group-hover:bg-primary group-hover:text-white group-hover:px-3 group-hover:py-1 group-hover:rounded-full">
                        Read More <i
                            class="fas fa-arrow-right ml-1.5 transition-transform duration-300 group-hover:translate-x-1"></i>
                    </a>
                </div>
            </div>
            @endforeach
