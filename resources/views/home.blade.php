    ...
    <div class="container mx-auto px-4 py-8">
        ...
        @else
            ...
            <div class="text-center mb-12 border-b-2 border-border pb-8">
                <h1 class="text-6xl md:text-8xl font-black text-text tracking-tighter leading-none mb-2 newspaper-title">The Simple News</h1>
            ...
            <div class="newspaper-layout">
                <div class="lead-story mb-8">
                    <a href="/post/{{ $lead->slug }}" class="block mb-8">
                        ...
                        <h1 class="text-4xl md:text-6xl font-bold text-text hover:text-primary transition duration-300">{{ $lead->title }}</h1>
                    ...
                    <div class="mb-6 pb-6 border-b border-border">
                        <a href="/post/{{ $post->slug }}">
                            <div class="aspect-w-16 aspect-h-9 rounded-lg overflow-hidden mb-3">
                                <img class="w-full h-full object-cover" src="{{ $post->image ? asset('storage/' . $post->image) : 'https://placehold.co/400x250/1e293b/94a3b8?text=Sub+Lead' }}" alt="{{ $post->title }}">
                            </div>
                            <h3 class="text-2xl font-semibold text-text hover:text-primary transition duration-300">{{ $post->title }}</h3>
                ...
                @foreach($topStories as $post)
                    <div class="border-b border-border py-4">
                        <a href="/post/{{ $post->slug }}">
                            <h3 class="text-xl font-semibold text-text hover:text-primary transition duration-300">{{ $post->title }}</h3>
            ...
            <section class="my-12 pt-8 border-t border-border">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach($moreNews as $post)
                        <div class="group bg-card p-4 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 border border-border">
                            <a href="/post/{{ $post->slug }}" class="block">
                                <div class="h-64 rounded-md overflow-hidden shadow-md mb-4">
                                    <img class="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-500" src="{{ $post->image ? asset('storage/' . $post->image) : 'https://placehold.co/300x200/1e293b/94a3b8?text=News' }}" alt="{{ $post->title }}">
                                </div>
                                <h3 class="text-lg font-semibold text-text group-hover:text-primary transition-colors duration-300">{{ $post->title }}</h3>
                            </a>
                        </div>
                    @endforeach
                </div>
