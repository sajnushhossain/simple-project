                    ...
                    <div class="lg:col-span-2">
                        <article class="bg-card-background p-4 rounded-lg shadow-sm">
                            <a href="/post/{{ $featured->slug }}" class="block group">
                                ...
                                <h1 class="font-serif text-3xl md:text-4xl text-dark-text mb-2 group-hover:text-primary-red transition-colors leading-tight">
                                    {{ $featured->title }}
                                </h1>
                        ...
                        <div class="mt-4 space-y-4 border-t border-border-light pt-6">
                            ...
                            @foreach($moreNews->take(4) as $post)
                                <article class="border-b border-border-light pb-4 last:border-b-0">
                                    <a href="/post/{{ $post->slug }}" class="block group">
                                        <h5 class="font-serif text-base text-dark-text leading-tight mb-1 group-hover:text-primary-red transition-colors line-clamp-2">
                                            {{ $post->title }}
                                        </h5>
                                        <div class="text-xs text-light-text">
                                            {{ $post->created_at->diffForHumans() }}
                    ...
                    @if($category->posts->isNotEmpty())
                        <section class="mb-8 border-t border-border-light pt-8">
                            <div class="flex items-center justify-between mb-6">
                                ...
                                <a href="/blog?category={{ $category->slug }}" class="text-base text-primary-red hover:underline font-semibold">
                                    View All →
                                </a>
                            </div>
                                ...
                                <div class="lg:col-span-2">
                                    <article class="bg-card-background p-3 rounded-lg shadow-sm">
                                        <a href="/post/{{ $firstPost->slug }}" class="block group">
                                            ...
                                            <h3 class="font-serif text-l text-dark-text mb-2 group-hover:text-primary-red transition-colors leading-tight">
                                                {{ $firstPost->title }}
                                            </h3>
                                    ...
                                    @foreach($remainingPosts as $post)
                                        <article class="flex gap-4 pb-6 border-b border-border-light last:border-b-0">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <a href="/post/{{ $post->slug }}" class="block group">
                                                    <h4 class="font-serif text-base text-dark-text mb-1 group-hover:text-primary-red transition-colors line-clamp-3 leading-tight">
                                                        {{ $post->title }}
                                                    </h4>
                                                    <div class="text-xs text-light-text">
                                                        {{ $post->created_at->diffForHumans() }}
                            ...
                            @foreach($moreNews->slice(4) as $post)
                                <article class="bg-card-background p-4 rounded-lg shadow-sm">
                                    <a href="/post/{{ $post->slug }}" class="block group">
                                        ...
                                        <h4 class="font-serif text-xl text-dark-text leading-tight mb-2 group-hover:text-primary-red transition-colors line-clamp-3">
                                            {{ $post->title }}
                                        </h4>
                                        <div class="text-xs text-light-text">
                                            {{ $post->created_at->diffForHumans() }}
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6">
                            @foreach($posts->take(12) as $post)
                                <article class="flex gap-4 pb-6 border-b border-border-light last:border-b-0">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <a href="/post/{{ $post->slug }}" class="block group">
                                            <h4 class="font-serif text-lg text-dark-text mb-1 group-hover:text-primary-red transition-colors line-clamp-3 leading-tight">
                                                {{ $post->title }}
                                            </h4>
                                            <div class="text-xs text-light-text">
                                                {{ $post->created_at->diffForHumans() }}
